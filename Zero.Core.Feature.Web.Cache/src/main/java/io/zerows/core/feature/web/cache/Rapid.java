...
public interface Rapid<K, V> {
    ...
    static <TK, TV> Rapid<TK, TV> object(final String key, final int ttl) {
        ...
        // return Fn.po?lThread(P.CACHED_THREAD, () -> new RapidObject<TV>(key, ttl), RapidObject.class.getName() + key + ttl);
    }
    ...
    static <T> Rapid<String, T> user(final User user, final String rootKey) {
        ...
        // return Fn.po?lThread(P.CACHED_THREAD, () -> new RapidUser<Tool>(user, rootKey), RapidUser.class.getName() + String.valueOf(user.hashCode()) + rootKey);
    }
