...
public abstract class AbstractAggregator extends AbstractAction {
    ...
    /*
     * Aggregation to map, -> ConcurrentMap<String, Tool>
     * - Only one group
     */
    ...
    /*
     * Aggregation to single result, -> Tool
     */
    ...
    private <T> T aggregateBy(final String fieldName, final T defaultValue, final Function<Field, T> function) {
        ...
        if (Objects.isNull(aggrField)) {
            /*
             * Default value of current Tool
             */
            return defaultValue;
        } else {
            /*
             * Field -> Tool execution
             */
