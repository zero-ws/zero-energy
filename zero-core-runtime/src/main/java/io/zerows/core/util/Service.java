...
class Service {
    ...
    private Service(final Class<?> serviceCls, final ClassLoader loader) {
    ...
    private <T> T service() {
        ...
        T reference = Ut.service((Class<T>) this.serviceCls, this.loader);
        if (Objects.isNull(reference)) {
            // Fix Liquibase Issue:
            /*
             * [ HED ] Missed `HED` component in service loader: META-INF/services/io.vertx.up.experiment.mixture.HED
             * This issue happened only when run `mvn liquibase:update`, because the class running is standalone
             */
            reference = Ut.service((Class<T>) this.serviceCls, this.serviceCls.getClassLoader());
        }
        return reference;
    }

    //    private <Tool> Tool service(final Class<Tool> serviceCls, final ClassLoader classLoader) {
    //        /*
    ...
    //         */
    //        final ServiceLoader<Tool> loader = ServiceLoader.load(serviceCls, classLoader);
    //        /*
        ...
        try {
            ...
            if (null == this.loader) {
                 ...
                 * else if (loader == ClassLoaders.platformClassLoader()) {
                            // The app.zero.cloud classloader doesn't have a class path,
                            // but the scanned loader might.
