...
class UnityStore implements JobStore {
    ...
    private static final OLog LOGGER = Ut.Log.cache(UnityStore.class);
    /*
    ...
    public Set<Mission> fetch() {
        ...
        LOGGER.info(MessageOfJob.STORE.SCANNED, missions.size(), "Programming");

        final Set<Mission> storage = this.store.fetch()
            .stream()
            .filter(mission -> !mission.isReadOnly())
            .collect(Collectors.toSet());
        LOGGER.info(MessageOfJob.STORE.SCANNED, storage.size(), "Dynamic/Stored");
