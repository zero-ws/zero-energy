package io.zerows.core.cloud.atom;
...
import io.zerows.core.cloud.eon.HName;
...
public class AeonWrapper implements HWrapper, Serializable {
    ...
    public static HWrapper configure(final JsonObject configJ) {
        ...
        return CStoreCloud.CC_BOOT.pick(() -> new AeonWrapper(configuration), configuration.hashCode());
    }
    ...
    public <C> C starter(final Class<?> interfaceCls, final Vertx vertx, final Class<?> defaultCls) {
        ...
        final TEvent event = CStoreCloud.CCT_EVENT.pick(() -> {
            final TEvent instance = Ut.instance(instanceCls);
            instance.bind(vertx);
            return instance;
        }, instanceCls.getName());
        Ho.LOG.Aeon.info(getClass(), "Pick instance class {0} of {1} from component cached/pool.",
